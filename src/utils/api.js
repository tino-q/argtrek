...
export async function loginUser(email, password) {
  const url = buildUrl(`email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}`);
...
export async function getRaftingCount(email, password) {
  const url = buildUrl(`endpoint=rafting_count&email=${email}&password=${password}`);
...
export async function downloadVoucher(email, password) {
  ...
  return fetchWithLocalStorageCache(
    "voucher",
    async () => {
      const url = buildUrl(`endpoint=download_voucher&email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}`);
